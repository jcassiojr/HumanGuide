# TRABALHAR NESSE COM OUTRO EXEMPLO (BREST C)
...
# Extract the class probabilities for Kynopsis.present.
x.evaluate$probabilities <- 1 - unlist(treeresponse(x.model,
                                                   newdata=x.evaluate), use.names=F)[seq(1,nrow(x.evaluate)*2,2)]
# teste--------------------
#library(pROC) 
rocobj1 <- plot.roc(x.evaluate$Kyphosis, x.evaluate$probabilities,  
                    main="Statistical comparison", percent=TRUE, col="#1c61b6")
#rocobj2 <- lines.roc(x.evaluate$Kyphosis, x.evaluate$Age, percent=TRUE, col="#008600")
