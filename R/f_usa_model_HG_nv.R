...
#' df_pred_out -> dataframe com os dados de uso para obter a previsão
...
#library("lubridate")
#require("doMC")
f_usa_model_HG_nv <- function(df_tidy_in, df_usa_in,campo_in) {
    #df_tidy_in <- my.atua.class
    #df_usa_in <- my.dados.uso
    #campo_in <- 1
    ...
    usaClass <- as.factor(df_usa_in[,"target"]) # transformando em vetor de fatores de target
    usaClass <- factor(usaClass, levels = c("T","F")) # ordenando levels para "S" ser o primeiro
