...
# obtém dados do Human Guide para Energia Sutentável do Brasil
...
df_tidy_usu_hg <- f_tidy_scores_HG_geral(df_usu) # calcula os fatores de acordo com a puntuação
...
pca1 = prcomp(df_tidy_usu_hg[,12:19], scale. = TRUE, center = TRUE)
# scores obtidos
scores.usu <- as.data.frame(pca1$x)
my.scores.usu <- as.data.frame(cbind(ID = df_tidy_usu_hg$ID,
                                 nomerespondente = df_tidy_usu_hg$nomerespondente,
                                 sexo = df_tidy_usu_hg$sexo, 
                                 profissao.na.area.de = df_tidy_usu_hg$profissao.na.area.de, 
                                formacao.em = df_tidy_usu_hg$formacao.em,
                                scores.usu))
# eliminando PC8
my.scores.usu <-
    my.scores.usu %>%
...
```{r, echo=FALSE, warning=FALSE}
# FORMACAO x CLASSE
# OBTENDO OS CAMPOS PROFISSIONAIS ASSOCIADOS A INFORMAÇÃO PASSADA PELO USUÁRIO
...
my.PC.usu <- cbind(my.PC.Campo.t[1:7,1:10],my.scores.usu.t) # unindo em mesmo data frame scores de usuario e scores medios de baseline
...
my.df_cor <-
    my.df_cor %>%
    mutate(CFM.prev = as.numeric(as.vector(CFM)),
           CFQ.prev = as.numeric(as.vector(CFQ)),
           CCF.prev = as.numeric(as.vector(CCF)),
           COA.prev = as.numeric(as.vector(COA)),
           CJS.prev = as.numeric(as.vector(CJS)),
           CCP.prev = as.numeric(as.vector(CCP)),
           CSL.prev = as.numeric(as.vector(CSL)),
           CMA.prev = as.numeric(as.vector(CMA)),
           CCE.prev = as.numeric(as.vector(CCE)),
           CBS.prev = as.numeric(as.vector(CBS)))
