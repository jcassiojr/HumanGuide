---
title: "Análise Descritiva - Preditiva / Energia Sustentável - DRAFT"
author: "Cassio"
date: "March 3, 2016"
output: 
   html_document
...
Este documento descreve a análise preditiva, realizada com o objetivo de avaliar a acurácia do modelo baseado em assinaturas de campos profissionais a partir dos resultados do teste **Human Guide**.
Foram usados para fins de avaliação dos resultados do modelo os dados obtidos dos perfis no projeto realizado junto à empresa Energia Sustentável do Brasil tomado como baseline para a previsãode um respondente.

#### Estratégia da validação ####

A estratégia adotada para a validação seguiu os seguintes passos:

1. Foi calculada a assinatura de cada usuário que respondeu ao teste **Human Guide**
2. Foi obtida a assinatura de baseline da cada campo profissional segundo a classificação de Levenfus, a partir dos dados da amostra original usada para calibrar o modelo
3. A partir desta baseline, foram obtidos os campos profissionais previstos para cada usuário
4. Esta previsão foi comparada com a *ocupação* informada pelo usuário, sendo obtida uma acurácia da previsão realizada em relação à ocupação.
5. Esta mesma previsão foi comparada com a *formação* informada pelo usuário, sendo obtida uma acurácia da previsão realizada em relação à formação.
6. Foi criada uma matriz de confusão (explicada abaixo) para avaliar a acurácia do modelo em prever a possibilidade de mudança de ocupação do usuário pelo fato da sua ocupação não estar alinhada ocm o seu perfil.
...
# função que prepara os dados, tirando acentos, brancos em excesso 
...
##################################################################
#df_usu <- read.xlsx2("./data/PP_09029666000490_20160302.xlsx", sheetIndex = 1, header = TRUE)
...
df_tidy_usu_hg <- f_tidy_scores_HG(df_raw_hg) # calcula os fatores de acordo com a puntuação

# chama funcao que tira acentos e força minúsculos
#df_tidy_hg <- f_acentos(df_tidy_hg)


#df_tidy_usu_hg <- f_tidy_scores_HG(df_usu) # calcula os fatores de acordo com a pontuação

df_tidy_usu_hg <- f_acentos(df_tidy_usu_hg) # tirando acentos

unfactorize<-c(7:14) # changing factor to numeric
...
pca1 = prcomp(df_change_usu[,5:12], scale. = TRUE, center = TRUE)
# scores obtidos
...
my.PC.Campo.t <- read.xlsx2("./data/BaselineAssinaturaCamposProfisisonais.xlsx", 
                    sheetIndex = 1, colIndex = c(2:12), header = TRUE)
...
prop.table(table(my.df_cor$hit.ocup)) # a acurácia foi baixa aqui: 45.5%
# acurácia de hit de formacao (muito baixa!)
table(my.df_cor$hit.form)
prop.table(table(my.df_cor$hit.form))  # a acurácia foi baixa aqui: 45.3%

# CONCLUSÃO
# os 7 PCs não são bom preditores para o campo profisisonal do respondente, mostrando acurácia abaixo de 50%
# como foi feito:
# 1. correlacionei as assinaturas dos respondentes com as do baseline de campos profissionais para obter os campos previstos
# 2. bati com os campos informados para medir a acurácia
# 3. idem para formação informada
# FALTA: mesma análise mas somente com 3 primeiros componntes
# OU: CFM tem compondnete 2 bem definido. Seleciono dos respondentes quem tem PC2 dentro do intervalo de confiança do
# CFM baseline! (my.PC.Campo.t)
# CFM - PC2 = -0.338206010
# CFM - SD = -0.338206010 +_ 1.255622000
# filtrar todos os respondentes que possuem score CFM nesta faixa (my.scores.usu.t)

# ANALISE PARA CFM (PC1) (ALTERNATIVA AO DE CIMA PARA VER SE TEM MAIS ACURACIA)
tst <- as.data.frame(t(my.PC.usu))
CFM.base <- tst[1:7,]
# cada coluna é um PC
names(CFM.base) <- c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7")

ocup.prev <- tst[11:dim(tst)[1],]
names(ocup.prev) <- c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7")
# aqui: criar coluna TRUE/FALSE para CFM.prev
ocup.prev <-
    ocup.prev %>%
    mutate(CFM.prev = ifelse(PC2 >= -1.59382801 & PC2 <= 0.91741599, TRUE, FALSE))

# para CFM vamos filtrar todos os respondentes que tem PC2 (coluna 2) na faixa acima)
my.df_CFM.prev <- CFM.tst %>%
    filter (PC2 >= -1.59382801 & PC2 <= 0.91741599)
# medir a acurácia de prever CFM

# ANALISE PARA CFQ (PC2 e PC4)
prev.tst <- as.data.frame(t(my.PC.usu))
# cada coluna é um PC
names(prev.tst) <- c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7")
# para CFM vamos filtrar todos os respondentes que tem PC2 (coluna 2) na faixa acima)
my.df_CFQ.prev <- prev.tst %>%
    filter (PC2 >= -1.59382801 & PC2 <= 0.91741599)
# criterio: score medio na faixa de .1 a -.1
# ANALISE PARA CCF (PC1, PC2 e PC3)
# ANALISE PARA COA (PC1 e PC2)
# ANALISE PARA CJS (PC1, PC2 e PC3)
# ANALISE PARA CCP (PC1, PC2, PC3 e PC4)
# ANALISE PARA CSL (PC4)
# ANALISE PARA CMA (PC1 e PC4)
# ANALISE PARA CCE (PC1 e PC3)
# ANALISE PARA CBS (PC1, PC2 e PC3)

# OBS: acima, outra analise de acuracia: fazer acurácia de cada campo para ver se os mais definidos na
# baseline tem maior acuracia
