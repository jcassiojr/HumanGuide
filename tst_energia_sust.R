...
f_compara_HG <- function(in.nome) {
    ...
    # plot
    plot.pessoa.ant <- ggplot(my.PCs.ant, aes(x=componente, y=valor)) +
    ...
    #my.score.limites$scores.resp.ant <- as.numeric(df_PCs)
    #nome.resp <- "Alex"
    
    # se score é negativo, calcula o percentual em relação ao mínimo e vice-versa
    #my.classif <- 
    #    my.score.limites %>%
    #    mutate(perc = abs(ifelse(scores.resp.ant <= 0, -(scores.resp.ant/min.score), (scores.resp.ant/max.score))))
    ...
    plot.pessoa <- ggplot(my.classif, aes(x=componente, y=perc)) +
